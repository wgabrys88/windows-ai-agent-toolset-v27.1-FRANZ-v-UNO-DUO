...
# This was a good starter! time to go to test on the beast 1060!
...
# -------------------------
# Robust tool arg handling (small-model + LM Studio hardening)
# -------------------------
_TOOL_SET = {"click", "double_click", "right_click", "drag", "type_text", "scroll", "attend"}
_COORD_PAT = re.compile(r"\b(x1|y1|x2|y2|x|y|dy)\s*[:=]\s*(-?\d+)\b", re.IGNORECASE)
...
def _extract_from_values(args: dict[str, Any], key: str) -> int | None:
    # direct
    if key in args:
        return _coerce_int(args[key])

    # scan string values for patterns like "y:560" (common failure: {"sync":"y:560"})
    for v in args.values():
        if isinstance(v, str):
            m = re.search(rf"\b{re.escape(key)}\s*[:=]\s*(-?\d+)\b", v, re.IGNORECASE)
            if m:
                return int(m.group(1))
            for k2, n2 in _COORD_PAT.findall(v):
                if k2.lower() == key.lower():
                    return int(n2)

    # scan raw dump if present
    raw = args.get("__raw")
    if isinstance(raw, str):
        for k2, n2 in _COORD_PAT.findall(raw):
            if k2.lower() == key.lower():
                return int(n2)
...
def normalize_tool_call(tool: str, args: Any, fallback_story: str) -> tuple[str, dict[str, Any]]:
    ...
    if not story:
        story = (fallback_story or DEFAULT_HUD_TEXT).strip()

    if tool in {"click", "double_click", "right_click"}:
        x = _extract_from_values(args, "x")
        y = _extract_from_values(args, "y")
        if x is None or y is None:
            return "attend", {"targets": [{"x": 500, "y": 500, "label": "format_fix_missing_xy"}], "story": story}
        return tool, {"x": clamp(int(x), 0, 1000), "y": clamp(int(y), 0, 1000), "story": story}

    if tool == "drag":
        x1 = _extract_from_values(args, "x1")
        y1 = _extract_from_values(args, "y1")
        x2 = _extract_from_values(args, "x2")
        y2 = _extract_from_values(args, "y2")
        if None in (x1, y1, x2, y2):
            return "attend", {"targets": [{"x": 500, "y": 500, "label": "format_fix_bad_drag"}], "story": story}
        return tool, {
    ...
    if tool == "scroll":
        dy = _extract_from_values(args, "dy")
        if dy is None:
            return "attend", {"targets": [{"x": 500, "y": 500, "label": "format_fix_bad_scroll"}], "story": story}
        return tool, {"dy": clamp(int(dy), -3000, 3000), "story": story}

    if tool == "type_text":
        text = str(args.get("text") or "").strip()
        if not text:
            return "attend", {"targets": [{"x": 500, "y": 500, "label": "format_fix_empty_text"}], "story": story}
        return tool, {"text": text, "story": story}

    # attend: preferred multi-target format + legacy compatibility
    targets: list[dict[str, Any]] = []
    raw_targets = args.get("targets")
    if isinstance(raw_targets, list):
        for t in raw_targets[:OBS_MAX_TARGETS]:
            if not isinstance(t, dict):
                t = {"__raw": str(t)}
            tx = _extract_from_values(t, "x")
            ty = _extract_from_values(t, "y")
            label = str(t.get("label") or "").strip() or "observe"
            if tx is None or ty is None:
                # salvage from item text if possible
                raw_item = json.dumps(t, ensure_ascii=False)
                tx = _extract_from_values({"__raw": raw_item}, "x") or 500
                ty = _extract_from_values({"__raw": raw_item}, "y") or 500
                label = "recovered_target"
            targets.append({"x": clamp(int(tx), 0, 1000), "y": clamp(int(ty), 0, 1000), "label": label})

    # legacy single-point format: {"x":..,"y":..,"label":..}
    if not targets:
        x = _extract_from_values(args, "x")
        y = _extract_from_values(args, "y")
        label = str(args.get("label") or "").strip() or "observe"
        if x is None or y is None:
            x, y = 500, 500
            label = "observe_default"
        targets = [{"x": clamp(int(x), 0, 1000), "y": clamp(int(y), 0, 1000), "label": label}]

    return "attend", {"targets": targets, "story": story}


def call_vlm(png: bytes, step: int) -> tuple[str, dict[str, Any]]:
    payload = {
        "model": MODEL_NAME,
        "messages": [
    ...
    if isinstance(args_raw, str):
        raw = _strip_code_fences(args_raw)
        try:
            parsed = json.loads(raw)
            args = parsed if isinstance(parsed, dict) else {"__raw": raw}
        except Exception:
            args = {"__raw": raw}
    elif isinstance(args_raw, dict):
        args = args_raw
    else:
        args = {"__raw": str(args_raw)}
        ...
        while not hud.stop.is_set():
            ...
            try:
                if test_mode:
                    ...
                    tool, args = call_vlm(png, step)
            except Exception as e:
                print(f"[{ts}] {step:03d} | VLM ERROR: {e}")
                time.sleep(1.0)
                continue

            fallback_story = hud.get_text()
            tool, args = normalize_tool_call(tool, args, fallback_story)
