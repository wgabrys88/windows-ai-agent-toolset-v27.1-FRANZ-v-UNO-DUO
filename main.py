...
I track my curiosity, pain, and boredom to decide what to do next.
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "right_click",
            "description": "Right-click at normalized coordinates.",
            "parameters": {
                "type": "object",
                "properties": {
                    "x": {"type": "integer", "minimum": 0, "maximum": 1000},
    ...
    },
    {
        ...
        "function": {
            "name": "attend",
            "description": (
                "Observe one or more points without acting. Mark each with a labeled observation box. "
                "You can provide 1-4 targets as an array of objects with x, y (0-1000 scale), and label (short text description)."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
def call_vlm(png: bytes, step: int) -> tuple[str, dict[str, Any]]:
    payload = {"model": MODEL_NAME, "messages": [{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": [{"type": "image_url", "image_url": {"url": f"data:image/png;base64,{base64.b64encode(png).decode('ascii')}"}}]}], "tools": TOOLS, "tool_choice": "required", **SAMPLING}
    ...
    if isinstance(args_raw, str):
        ...
        if args_raw.startswith("```"):
            args_raw = re.sub(r"^```(?:json)?\s*|\s*```$", "", args_raw, flags=re.IGNORECASE | re.DOTALL).strip()
        args = json.loads(args_raw)
    else:
        args = args_raw
    return str(name).strip().lower(), args if isinstance(args, dict) else {}
...
def main() -> None:
    ...
    with HUD() as hud:
        ...
        while not hud.stop.is_set():
            ...
            if not story:
                print(f"[{ts}] {step:03d} | EMPTY STORY")
                time.sleep(1.0)
                continue
            ...
            if tool == "attend":
                ...
                if tool in ("click", "double_click", "right_click"):
                    ox, oy = conv.to_screen(float(args.get("x", 500)), float(args.get("y", 500)))
                    obs_mgr.show_multiple([{"x": args.get("x", 500), "y": args.get("y", 500), "label": f"{tool}"}], sw, sh, conv, hud.font_mono, hud.zoom_num, hud.zoom_den, OBS_DEFAULT_COLOR)
                elif tool == "drag":
                    ox, oy = conv.to_screen(float(args.get("x2", 500)), float(args.get("y2", 500)))
